  ...
  async _detectVersions (schemas) {
    for (let schema of schemas) {
      if (!schema.scripts.version) {
        this.logger.warn('Version script for ' + name + ' not found, skipping...')
        continue
      }

      if (!schema.scripts.validate) {
        this.logger.warn('Validate script for ' + name + ' not found, skipping...')
        continue
      }

      try {
        const [languageName, languageVersion, implementationName, implementationVersion] = (await childProcess.execFile(
